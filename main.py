...
class ChatBot:
    ...
    def common_args(self, update, context):
        if update.message.chat.type != "private":
            ...
            old = 0
            domisili = "biosfer"
        return user_id, name, username, old, domisili

    def start(self, update, context):
        user_id, name, username, old, domisili = self.common_args(update, context)
        ...
        if chat_type == "private":
            try:
                ...
                if not check_user:
                    # record insertion
                    self.record.insert(user_id, name, username, old, domisili)
                ...
                if update.old == 0:
                    self.old(update, context)
                if update.dom == 'biosfer':
    ...
    def umur(self, old, context):
        user_id, name, username = self.common_args(umur, context)

        # chat type (group or private)
        chat_type = umur.message.chat.type
